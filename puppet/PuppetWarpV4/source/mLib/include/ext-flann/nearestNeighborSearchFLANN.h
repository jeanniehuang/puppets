...
namespace ml
{
    ...
    public:
        ...
        void getDistances(unsigned int k, std::vector<FloatType> &dists) const override
        {
            dists = getDistances(k);
        }
    ...
    private:
        void initInternal(const std::vector< const FloatType* > &points, uint32_t dimension, uint32_t maxK) override
        {
        ...
        void kNearestInternal(const FloatType* query, uint32_t k, FloatType epsilon, std::vector<uint32_t> &result) const override
        {
            memcpy(m_queryStorage.data, query, m_dimension * sizeof(FloatType));
        ...
        void fixedRadiusInternal(const FloatType* query, uint32_t k, FloatType radius, FloatType epsilon, std::vector<uint32_t> &result) const override
        {
            memcpy(m_queryStorage.data, query, m_dimension * sizeof(FloatType));
        ...
        void fixedRadiusInternalDist(const FloatType* query, uint32_t k, FloatType radius, FloatType epsilon, std::vector< std::pair<uint32_t, FloatType> > &result) const override
        {
            memcpy(m_queryStorage.data, query, m_dimension * sizeof(FloatType));
            int res = m_FLANNIndex->radiusSearch(m_queryStorage, m_indicesStorage, m_distsStorage, radius*radius, flann::SearchParams((int)m_checkCount));
